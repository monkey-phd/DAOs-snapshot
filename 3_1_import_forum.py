...
def scarpe_selenium():
    ...
    for url in urls.itertuples():
        if url.exists and (not url.selenium_body or (type(url.selenium_body) != str and math.isnan(url.selenium_body))):
            print(url.url)
    ...
    for url in urls.itertuples():
        if url.exists and type(url.selenium_body) == str:
            ...
            if "<discourse-assets>" in url.selenium_body:
                ...
                data = json.loads(soup.select("discourse-assets-json > div")[0]["data-preloaded"])
...
def filter_usable_data():
    ...
    for i, url in enumerate(urls.itertuples()):
        data = json.loads(url.discourse_json)
        data["url"] = url.url
